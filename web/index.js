...
var App = function () {
	...
	self.features = {};
	self.features.fail2ban_enabled = ko.observable(false);
		...
		request(
			...
			function (data) {
				if (data.error === null) {
					...
					self.features.fail2ban_enabled(data.data.fail2ban_enabled);
				...
				if (data.error === null) {
					switch (data.data) {
						case 'user_not_blocked':
							showInfo(Translation.get('accountnotlocked'));
							break;
						case 'user_unblocked':
							showWarning(Translation.get('accountunlocked'));
							break;
					}
				} else {
					if (data.error === 'unblocking_not_enabled') {
						showError(Translation.get('unblockingnotenabled'));
					}
				}
			});
	};
